...
CGHostDBSQLite :: CGHostDBSQLite( CConfig *CFG ) : CGHostDB( CFG )
{
	...
	if (!exists(m_localf))
	{
		...
		if( m_local->Exec( "CREATE TABLE iptocountry ( ip1 INTEGER NOT NULL, ip2 INTEGER NOT NULL, country TEXT NOT NULL, countrylong TEXT NOT NULL, city TEXT NOT NULL, PRIMARY KEY ( ip1, ip2 ) )" ) != SQLITE_OK )
...
//		CONSOLE_Print( "[SQLITE3] prepare error looking for scores table - " + m_DB->GetError( ) );
	...
	if( m_DB->Exec( "CREATE TEMPORARY TABLE iptocountry ( ip1 INTEGER NOT NULL, ip2 INTEGER NOT NULL, country TEXT NOT NULL, countrylong TEXT NOT NULL, city TEXT NOT NULL, PRIMARY KEY ( ip1, ip2 ) )" ) != SQLITE_OK )
...
/* string CGHostDBSQLite :: CountryCityCheck( uint32_t ip )
{
	// a big thank you to Gen for help with the iptocity feature

	string From = string();
	sqlite3_stmt *Statement;
	m_DB->Prepare( "SELECT countrylong, city FROM iptocountry WHERE ip1<=? AND ip2>=?", (void **)&Statement );
	...
	if( Statement )
	{
		...
		while( RC == SQLITE_ROW )
		{
			...
			if( Row->size( ) == 2 )
				...
				CONSOLE_Print( "[SQLITE3] error checking iptocountry [" + UTIL_ToString( ip ) + "] - row doesn't have 2 columns" );
			RC = m_DB->Step( Statement );
		}
...
string CGHostDBSQLite :: CountryCheck( uint32_t ip )
{
	// a big thank you to Gen for help with the iptocity feature

	string From = "";
	sqlite3_stmt *Statement;
	m_DB->Prepare( "SELECT countrylong FROM iptocountry WHERE ip1<=? AND ip2>=?", (void **)&Statement );
...
string CGHostDBSQLite :: CityCheck( uint32_t ip )
{
	// a big thank you to Gen for help with the iptocity feature

	string From = "";
	sqlite3_stmt *Statement;
	m_DB->Prepare( "SELECT city FROM iptocountry WHERE ip1<=? AND ip2>=?", (void **)&Statement );
...
bool CGHostDBSQLite :: FromAdd( uint32_t ip1, uint32_t ip2, string country, string countrylong, string city )
{
	// a big thank you to tjado for help with the iptocountry feature
	...
	if( FromAddStmt )
	{
		...
		sqlite3_bind_text( (sqlite3_stmt *)FromAddStmt, 4, countrylong.c_str( ), -1, SQLITE_TRANSIENT ); //Gen
		sqlite3_bind_text( (sqlite3_stmt *)FromAddStmt, 5, city.c_str( ), -1, SQLITE_TRANSIENT ); //Gen
