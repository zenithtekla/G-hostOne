...
//					P = m_GHost->UDPChatWhoIs((*i)->GetExternalIPString());
	...
	/*
				...
				if( ( Command == "startn" || Command == "startf" || Command == "forcestart" ) && !m_CountDownStarted )
				{
					...
					{
						SendAllChat( m_GHost->m_Language->SureForStartGame( ) );
						return HideCommand;
					}
				...
				if( Command == "rmk" && !player->GetRmkVote( ) && m_GameLoaded)
				{
					...
					{
						SendAllChat( m_GHost->m_Language->GameOverIn("5") );
						m_GameEndCountDownStarted = true;
						m_GameEndCountDownCounter = 5;
						m_GameEndLastCountDownTicks = GetTicks( );
			...
			{				
				...
				{		
					...
					if (m_GetMapNumTeams==2 || m_Map->GetMapType( ).find("2teams") != string::npos ) {
						...
						{
							SendAllChat( m_GHost->m_Language->BothTeamsMustContainAPlayer( ) );
							return HideCommand;
						}
...
//
	...
	if( Command == "wim" && !m_CountDownStarted )
	{
		...
		if(!wiim.empty())
			...
			SendAllChat( m_GHost->m_Language->PlayersNeedToReady(wiim) );
		else 
			SendAllChat( m_GHost->m_Language->PlayersPinged(UTIL_ToString( GetNumHumanPlayers( ) )));
	}
	...
	if( ( Command == "ready" || Command == "rdy" || Command == "go" ) && !m_CountDownStarted && !m_GameLoaded && !m_GameLoading && !m_StartVoteStarted && !player->GetStartVote( ) )
	{
		...
		player->SetStartVote( true );
		SendAllChat( m_GHost->m_Language->PlayerReady(User));		
	}
	...
	else if( ( Command == "ready" || Command == "rdy" || Command == "go" ) && !m_CountDownStarted && !m_GameLoaded && !m_GameLoading && m_StartVoteStarted && !player->GetStartVote( ) )
	{
		...
		if( sVotes >= sVotesNeeded )
		{
			...
			SendAllChat( m_GHost->m_Language->EverybodyReady( ) );			
			StartCountDown( true );
		}
	}
	else if( ( Command == "ready" || Command == "rdy" || Command == "go" ) && !m_CountDownStarted && !m_GameLoaded && !m_GameLoading && m_StartVoteStarted && player->GetStartVote( ) )
		SendChat( player, m_GHost->m_Language->PlayerIsReady( ) );
	//
	// !RMK
	//
    ...
    if( Command == "rmkcounter" && m_GameLoaded) 
	{
      //count RMK's
        ...
        SendChat(player, m_GHost->m_Language->RMKCounter( ) + " " + UTIL_ToString( VotesRMK1 ) + "/" + UTIL_ToString( CountRMK1));
	};
	...
	if( (Command == "rmk" || Command == "remake") && m_GameLoaded && player->CanRMK()) 
	{
		...
		if(player->GetHasRMKed()) 
		{
			if (!team_p) {
			//sentinel
				SendAllChat( m_GHost->m_Language->VoteForRMK(User) + " " + UTIL_ToString( VotesRMK ) + "/" + UTIL_ToString( CountRMK ));
			}			
		} else
		{
            if (!team_p) {
				SendAllChat( m_GHost->m_Language->UnVoteForRMK(User) + " " + UTIL_ToString( VotesRMK ) + "/" + UTIL_ToString( CountRMK ));
			} 
		} 
		...
		{     
		  //game_RMK=true;  //disable autoban //autosave
			SendAllChat( m_GHost->m_Language->RMKSucceeded( ) );
			if (!m_GameEndCountDownStarted)
						{
...
//		player->SetRMKSentTime( GetTime( ) );
    ...
    uint32_t VotesSentinel1 = 0;
        ...
        SendChat(player,m_GHost->m_Language->FFCounter( UTIL_ToString( VotesSentinel1 ), UTIL_ToString( CountSentinel1), UTIL_ToString( VotesScourge1 ), UTIL_ToString( CountScourge1 )));
	...
	if( (Command == "ff" || Command == "forfeit") && m_GameLoaded && player->CanFF()) 
	{
      ...
      player->SetCanFF(false);      // w8 1 min
		...
		if(player->GetHasFFed()) 
		{
			if (!team_p) {
			//sentinel
				SendAllChat( m_GHost->m_Language->PlayerHasFF(User) + " Sentinel: " + UTIL_ToString( VotesSentinel ) + "/" + UTIL_ToString( CountSentinel ));
			} else {
			//scourge
				SendAllChat( m_GHost->m_Language->PlayerHasFF(User) + " Scourge: " + UTIL_ToString( VotesScourge ) + "/" + UTIL_ToString( CountScourge ));
		}			
		} else
		{
            if (!team_p) {
				SendAllChat( m_GHost->m_Language->PlayerHasOath(User) + " Sentinel: " + UTIL_ToString( VotesSentinel ) + "/" + UTIL_ToString( CountSentinel ));
			} else {
				SendAllChat( m_GHost->m_Language->PlayerHasOath(User) + " Scourge: " + UTIL_ToString( VotesScourge ) + "/" + UTIL_ToString( CountScourge ));
			};
		} 
		...
		if((VotesSentinel == CountSentinel) && (CountSentinel!=0) ) // has at least a player
		{
		  ...
		  //game_ff=true;  //disable autoban //autosave
			SendAllChat( m_GHost->m_Language->TheScourgeWin( ) );
			if (!m_GameEndCountDownStarted)
						{
							CONSOLE_Print( "[GAME: " + m_GameName + "] ending (admin can end the game now with !endn)" );
							SendAllChat(m_GHost->m_Language->GameOverIn("10"));
							m_GameEndCountDownStarted = true;
		...
		else if((VotesScourge == CountScourge) && (CountScourge!=0)) // has at least a player
		{
			m_Stats->SetWinner(1);    
			//game_ff=true;     //disable autoban  //autosave
			SendAllChat( m_GHost->m_Language->TheSentinelWin( ) );
			if (!m_GameEndCountDownStarted)
						{
							CONSOLE_Print( "[GAME: " + m_GameName + "] ending (admin can end the game now with !endn)" );
							SendAllChat(m_GHost->m_Language->GameOverIn("10"));
