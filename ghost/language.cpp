...
string CLanguage :: Publicity( string web, string groupid )
{
	...
	if ( Out.find("TheGenMaps.tk") == string::npos || Out.empty() )
		Out = "Visit TheGenMaps.tk for PRO gaming exp. Join Garena+ GroupID 56934.";
	return Out;
}
...
string CLanguage :: KickMsgForSlowDL( string name, string dlspeed, string dlquota )
{
	string Out = m_CFG->GetString( "lang_1041", "lang_1041" );
	Replace( Out, "$DLSPEED$", dlspeed);
	Replace( Out, "$LOWESTDLRATE$", dlquota);
	Replace( Out, "$NAME$", name );
	return Out;
}
...
{
	string Out = m_CFG->GetString( "lang_1044", "lang_1044" );
	Replace( Out, "$BAN$", ban );
	Replace( Out, "$TIMER$", timer );
	return Out;
}
string CLanguage :: KickMsgForAbuser( string name )
{
	string Out = m_CFG->GetString( "lang_1045", "lang_1045" );
	Replace( Out, "$NAME$", name );
	return Out;
}
