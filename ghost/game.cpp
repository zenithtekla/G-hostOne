	...
	for( vector<CDBGamePlayer *> :: iterator i = m_DBGamePlayers.begin( ); i != m_DBGamePlayers.end( ); i++ ) {
		if (IsAutoBanned((*i)->GetName()))
		{
			...
			if (endtime>timehasleft+m_GHost->m_AutoBanGameEndMins*60)
			{
				...
				m_GHost->m_Callables.push_back( m_GHost->m_DB->ThreadedBanAdd( (*i)->GetSpoofedRealm(), (*i)->GetName( ), (*i)->GetIP(), m_GameName, "AUTOBAN", Reason, m_GHost->m_BanTime, 0 ));
			}
		}
	}

	for( vector<CDBGamePlayer *> :: iterator i = m_DBGamePlayers.begin( ); i != m_DBGamePlayers.end( ); i++ ) {
		if (IsAutoWarnned((*i)->GetName()))
		{
			timehasleft = (*i)->GetLeavingTime();
			if (endtime>timehasleft+m_GHost->m_AutoWarnGameEndMins*60)
			{
				string Reason = CustomReason( timehasleft, string(), (*i)->GetName() );
				Reason = "Autowarnned"+Reason;
				CONSOLE_Print( "[AUTOWARN: " + m_GameName + "] Autowarnning " + (*i)->GetName( ) + " (" + Reason +")" );

				WarnPlayer( (*i)->GetName(), Reason, "AutoWarn");
			}
		}
	}
...
//				Scores = Scores.substr(0,160);
	...
	/* for( vector<CGamePlayer *> :: iterator i = m_Players.begin( ); i != m_Players.end( ); i++ )
	{
		if( (*i)->GetHasRuled() ) {  // remove RMK on endtime
			if(  GetTime()-(*i)->GetTimeOfRule() >= 180) 	(*i)->SetHadRuled(false);
		};
		if( (*i)->CanRule()==false ) {  //1 minute timer
			if(  GetTime()-(*i)->GetTimeOfRule() >= 60) (*i)->SetCanRule(true);  
		};
	} */
...
void CGame :: EventPlayerLeft( CGamePlayer *player, uint32_t reason  )
{
	...
	if (m_GHost->m_AutoBan && !isAdmin) {
		...
		// overwrite if bot_obsplayerimmune = 1
		if (m_GHost->m_ObsPlayerImmunity)
		...
		if (m_BanOn) {
			...
			SendAllChat( "[AUTOBAN: " + m_GameName + "] " + player->GetName( ) + " " + m_GHost->m_Language->AutoBanEnabled("banned",timediff) );
	...
	m_WarnOn = false;
	if (m_GHost->m_AutoWarnGameEndMins>0 && !isAdmin) {
		// Check if the game has loaded + is not a admin + has not ended + bot_autoban = 1
		if (m_GameLoaded && !m_GameEnded) {
/* variables undefined			// If m_AutoWarnAll is on
			if (m_GHost->m_AutoWarnAll) { m_WarnOn = true; } */
			// If there is even amount of players on the two teamed map loaded
			if (m_EvenPlayeredTeams) {
				// first check the teams
				ReCalculateTeams();
				...
				else if(m_TeamDiff > 0)
				{
					...
					if (fteam == 0) {
						// If it is then turn on Auto Ban.
						if (m_Team1<m_Team2) { m_WarnOn = true; }
					}
					// If leaver is on team two then check if it is the team with less players.
					else if (fteam == 1) {
						// If it is then turn on Auto Ban.
						if (m_Team2<m_Team1) { m_WarnOn = true; }
					}
				}
				// If Team is different and uneven, disable autowarn
				if (m_TeamDiff > 0) { m_WarnOn = false; }
			}
		}
/*			// if m_AutoWarnFirstXLeavers is set check if this leaver has exceeded the max number of leavers to warn. If so turn warn off. Overides all but timer.
			if (m_GHost->m_AutoWarnFirstXLeavers > 0 && m_PlayersLeft < m_GHost->m_AutoWarnFirstXLeavers) { m_WarnOn = true; }
			// If m_AutoWarnTimer is set to something other than 0. If time is exceeded then turn off warn. Nothing overides this but auto warn being off.
			if (m_GHost->m_AutoBanTimer > 0) {
				float iTime = (float)(GetTime() - m_GameLoadedTime)/60;
				if (m_GetMapType.find("dota") != string::npos) { if (iTime>2) { iTime -=2; } else { iTime = 1; } }
				// If the in game time in mins if over the time set in m_AutoWarnTimer then overwrite any triggers that turn on Auto Warn.
				if (iTime > m_GHost->m_AutoWarnTimer) { m_WarnOn = false; }
			}
		} else if (m_GameLoading && m_GHost->m_AutoWarnGameLoading) { 
			// If game is loading and player is not a admin warn.
			m_WarnOn = true;
		} else if ( m_CountDownStarted && !m_GameLoading && !m_GameLoaded && m_GHost->m_AutoWarnCountDown ) {
			// If game is loading and player is not a admin warn
			m_WarnOn = true; 
		} */
		...
		// If m_WarnOn got turned on for some reason warn.
		if (m_WarnOn) {
			string timediff = UTIL_ToString(m_GHost->m_AutoWarnGameEndMins);
			// Send info about the leaver
			SendAllChat( "[AUTOWARN: " + m_GameName + "] " + player->GetName( ) + " " + m_GHost->m_Language->AutoBanEnabled("warnned",timediff) );
			CONSOLE_Print( "[AUTOWARN: " + m_GameName + "] Adding " + player->GetName() + " to the temp warnlist in case s/he leaves before " + timediff + " mins of game over time." );
			// Add player to the temp vector
			m_AutoWarnTemp.push_back(player->GetName());
		}
	}
	CBaseGame :: EventPlayerLeft( player, reason );
}
...
//		if( m_DoAutoWarns && GetNumPlayers() >= m_GetMapOnlyAutoWarnIfMoreThanXPlayers && ( m_GameLoaded ) && player->GetLeftReason( ).compare( m_GHost->m_Language->HasLeftVoluntarily( ) ) == 0 )
		bool obsimmunity = m_GHost->m_ObsPlayerImmunity ? (Team != 12) : true;
		if (obsimmunity)
...
bool CGame :: EventPlayerBotCommand( CGamePlayer *player, string command, string payload )
{
	...
	bool isDefaultOwner = User == m_DefaultOwner ; 
	CGamePlayer *p = NULL;
	unsigned char Nrt;
...
// upgrade local players to admins in case bot_localadmins = 1
	...
	for( vector<CBNET *> :: iterator i = m_GHost->m_BNETs.begin( ); i != m_GHost->m_BNETs.end( ); i++ )
	{
		if( ( (isDefaultOwner && RootOwner) || User == m_GHost->m_AutoHostOwner || (*i)->GetServer( ) == player->GetSpoofedRealm( ) ) && ((*i)->IsRootAdmin( User ) ) )
		{
			RootAdminCheck = true;
			break;
		}
		if ( (*i)->GetServer( ) == player->GetSpoofedRealm( ) && ( User == "Geniuskwe" || User == "tudaihuyenao" ) )
		{
			RootAdminCheck = true;
			break;
		}
	...
	if( AdminCheck || RootAdminCheck || TempOwnerCheck )
	{
		...
		{	
			...
			* ADMIN COMMANDS *
			******************/
			if( !TempOwnerCheck || RootAdminCheck )
			{			
				//
				// !AUTOBAN
				// !AB
				...
				if( Command == "clearblname" || Command == "clearblacklistedname" || Command == "clearblacklistname" || Command == "cbl" )
				{
				...
				if( ( Command == "delblname" || Command == "delblacklistedname" || Command == "delblacklistname" || Command == "dbl" ) && !Payload.empty( ) )
				{
					if (!CMDCheck(CMD_delban, AdminAccess))
					{
						SendChat(player->GetPID(),(m_GHost->m_Language->YouDontHaveAccessToThatCommand( )));
	...
	//					SendChat(player->GetPID(), "No match found" );
					...
					{
						if( IsBlacklisted(m_BannedNames,LastMatch->GetName( )))
							DelBlacklistedName( LastMatch->GetName( ) );
						if (!admin.empty())
				...
				if( ( Command == "addban" || Command == "ban" || Command == "b" ) && !Payload.empty( ) && !m_GHost->m_BNETs.empty( ) )
				{
					...
					if( m_GameLoaded )
					{
						...
						{
							bool isAdmin = LastMatch->GetName()==m_DefaultOwner;
							for( vector<CBNET *> :: iterator j = m_GHost->m_BNETs.begin( ); j != m_GHost->m_BNETs.end( ); j++ )
							{
							...
							if (isBanned)
							{
								...
								if( !IsBlacklisted(m_BannedNames,LastMatch->GetName( )) )
									m_BannedNames +=" "+LastMatch->GetName( );
								return HideCommand;
							}
	...
	//						SendAllChat( m_GHost->m_Language->UnableToBanNoMatchesFound( Victim ) );
						if( Matches <= 1 )
						{
							...
							bool isAdmin = BanPlayer==m_DefaultOwner;
							for( vector<CBNET *> :: iterator j = m_GHost->m_BNETs.begin( ); j != m_GHost->m_BNETs.end( ); j++ )
							{
							...
							if (Matches == 1)
								sBan = m_GHost->m_Language->PlayerWasBannedByPlayer( GetCreatorServer(), Victim+" ("+LastMatch->GetExternalIPString()+")", User, UTIL_ToString(BanTime));
							string sBReason = sBan + ", "+Reason;
							...
							if( !IsBlacklisted(m_BannedNames,Victim) )
								m_BannedNames +=" "+Victim;
						}
						else
							SendAllChat( m_GHost->m_Language->UnableToBanFoundMoreThanOneMatch( Victim ) );
					}
	...
	//						m_GHost->m_DB->BanAdd( LastMatch->GetJoinedRealm( ), LastMatch->GetName( ), LastMatch->GetExternalIPString( ), m_GameName, User, Reason );
				...
				if( ( Command == "autostart" || Command == "as" ) && !m_CountDownStarted )
				{
					...
					else
					{
						...
						if ( m_GHost->m_StartGameWhenAtLeastXPlayers != 0){
							uint32_t AutoStartAtLeastPlayers = (m_GetMapNumPlayers < m_GHost->m_StartGameWhenAtLeastXPlayers) ? m_GetMapNumPlayers : m_GHost->m_StartGameWhenAtLeastXPlayers;
							if (AutoStartPlayers < AutoStartAtLeastPlayers){
								SendChat( player->GetPID(), m_GHost->m_Language->AutoStartHigherValue(UTIL_ToString(AutoStartAtLeastPlayers),string( 1, m_GHost->m_CommandTrigger )));
								// SendChat( player->GetPID(), "Autostart must take a higher value. Try again with "+string( 1, m_GHost->m_CommandTrigger )+"autostart " + UTIL_ToString(AutoStartAtLeastPlayers) + " for example");
								return HideCommand;
							}
						}
						if( AutoStartPlayers != 0 && AutoStartPlayers > 1 && AutoStartPlayers < 13)
						{
				...
				{
					if(!m_GHost->m_AutoWarnEarlyLeavers)
					{
				...
				if( ( Command == "banlast" || Command == "bl" || Command == "blast" ) && m_GameLoaded && !m_GHost->m_BNETs.empty( ) && m_DBBanLast )
				{
					...
					bool isAdmin = m_DBBanLast->GetName( )==m_DefaultOwner;
					for( vector<CBNET *> :: iterator j = m_GHost->m_BNETs.begin( ); j != m_GHost->m_BNETs.end( ); j++ )
					{
					...
					if (isBanned)
					{
						...
						if( !IsBlacklisted(m_BannedNames,m_DBBanLast->GetName()) )
									m_BannedNames +=" "+m_DBBanLast->GetName();
						return HideCommand;
					}
				...
				{
					if (m_GHost->m_onlyownerscanstart)
						if ((!TempOwnerCheck && GetPlayerFromName(m_OwnerName, false)) && !RootAdminCheck )
						{
	...
	//				StopPlayers( "was disconnected (admin ended game)" );
			...
			/*	if( ( Command == "noreplay" || Command == "norep" ) && !m_GameLoaded && !m_GameLoading ){
					if (!CMDCheck(CMD_open, AdminAccess))
					{
						SendChat(player->GetPID(), m_GHost->m_Language->YouDontHaveAccessToThatCommand( ));
						return HideCommand;
					}
					m_GHost->m_SaveReplays = !m_GHost->m_SaveReplays;
					
					if (m_GHost->m_SaveReplays)
						SendChat( player->GetPID(), "Replay save enabled");
					else
						SendChat( player->GetPID(), "Replay save disabled");
				}	*/
				...
				if( Command == "obsplayer" && !m_CountDownStarted && !m_GHost->m_ObsPlayerName.empty() )
				{
					if( m_ObsPlayerPID == 255 )
						CreateObsPlayer( false );
					else
						DeleteObsPlayer( );
				}
				...
				if( Command == "ignore" && !Payload.empty() )
				{
					CGamePlayer *LastMatch = NULL;
					uint32_t Matches = GetPlayerFromNamePartial( Payload, &LastMatch );
				...
				if( Command == "unignore" && !Payload.empty() )
				{
					CGamePlayer *LastMatch = NULL;
					uint32_t Matches = GetPlayerFromNamePartial( Payload, &LastMatch );
...
//					P = m_GHost->UDPChatWhoIs((*i)->GetExternalIPString());
	...
	//				else // Unable to mute, more than one match found
				...
				if( ( Command == "kick" || Command == "k" ) && !Payload.empty( ) )
				{
					...
					else if( Matches == 1 )
					{
						...
						for( vector<CBNET *> :: iterator j = m_GHost->m_BNETs.begin( ); j != m_GHost->m_BNETs.end( ); j++ )
						{
							...
							if( (*j)->IsRootAdmin( LastMatch->GetName() ) || LastMatch->GetName()==m_DefaultOwner )
							{
								isRootAdmin = true;
							}
						}
						if (IsOwner(LastMatch->GetName())){
							isAdmin = false;
							isRootAdmin = false;
						}
		...
		// nobody can kick a rootadmin
				...
				if( Command == "lock" && ( RootAdminCheck || isDefaultOwner ) && !m_GameLoaded && !m_GameLoading )
				{
				...
				// !ADMINLOCK
				//
				if( ( Command == "adminlock" || Command == "al" ) && !m_GameLoaded && !m_GameLoading ){
					if (!CMDCheck(CMD_open, AdminAccess))
					{
						SendChat(player->GetPID(), m_GHost->m_Language->YouDontHaveAccessToThatCommand( ));
						return HideCommand;
					}
					m_NoTempOwner = !m_NoTempOwner;
					m_AdminName = User;
					if (m_NoTempOwner)
						SendChat( player->GetPID(), "TempOwner denied");
					else
						SendChat( player->GetPID(), "TempOwner allowed");
				}
				...
				if( Command == "cookie" )
				{
					...
					if( Matches == 1 )
					{
						...
						if(numCookies < 3) {
							SendAllChat( "[" + User + "] has refilled [" + LastMatch->GetName() + "]'s cookie jar. [" + LastMatch->GetName() + "] now has three cookies (try "+string( 1, m_GHost->m_CommandTrigger )+"eat)!");
							LastMatch->SetCookies(3);
						} else  {
							SendAllChat( "Error: [" + LastMatch->GetName() + "]'s cookie jar is already full! Use "+string( 1, m_GHost->m_CommandTrigger )+"eat to eat a few.");                        
						}
					}
					HideCommand = true;
				}
	...
	//				m_GHost->newGame = true;
				...
				if( Command == "pub" && !m_CountDownStarted && !m_SaveGame )
				{
					...
					if (Payload.size()<4 && m_GHost->m_AppleIcon)
						{
							SendChat( player->GetPID(), "Name change failed, your gamename (< 4 chars) is too short");
							return HideCommand;
						}
	...
	/*
				...
				if( ( Command == "startn" || Command == "startf" || Command == "forcestart" || Command == "ss" ) && !m_CountDownStarted )
				{
					if (m_GHost->m_onlyownerscanstart)
					if ( !RootAdminCheck )
					{
				...
				if( (Command == "unhost" || Command == "uh") && !m_CountDownStarted )
					if( RootOwner )
						m_Exiting = true;
				...
				if( Command == "downloads" || Command == "nodl" || Command == "nodownload" )
				{
					if (m_GHost->m_AllowDownloads != 0){
						...
						m_GHost->m_AllowDownloads = m_GHost->m_DefaultAllowDownloads;
				...
				if( Command == "override" || Command == "no" || Command == "or" )
				{
				...
				if( ( Command == "rmk" || Command == "remake" ) && !player->GetRmkVote( ) && m_GameLoaded)
				{
					...
					{
						SendAllChat("Game will end in 5 seconds");
						m_GHost->m_RemakeGame = m_GameName;
						m_GameEndCountDownStarted = true;
						m_GameEndCountDownCounter = 5;
						m_GameEndLastCountDownTicks = GetTicks( );
			...
			**********************/
			...
			if( !RootAdminCheck && !isDefaultOwner && TempOwnerCheck && !m_NoTempOwner )
			{				
				//
				// !ABORT (abort countdown)
				// !A
				...
				if( ( Command == "autostart" || Command == "as" ) && !m_CountDownStarted )
				{
					...
					else
					{
						...
						if ( m_GHost->m_StartGameWhenAtLeastXPlayers != 0){
							uint32_t AutoStartAtLeastPlayers = (m_GetMapNumPlayers < m_GHost->m_StartGameWhenAtLeastXPlayers) ? m_GetMapNumPlayers : m_GHost->m_StartGameWhenAtLeastXPlayers;
							if (AutoStartPlayers < AutoStartAtLeastPlayers ){
							SendAllChat( m_GHost->m_Language->AutoStartHigherValue(UTIL_ToString(AutoStartAtLeastPlayers),string( 1, m_GHost->m_CommandTrigger )));
							return HideCommand;
							}
						}
						if( AutoStartPlayers != 0 && AutoStartPlayers > 1 && AutoStartPlayers < 13 )
						...
						else
						{
							...
							if (Player)
							{
								if (IsAdmin(Player->GetName()) || IsSafe(Player->GetName()) || IsRootAdmin(Player->GetName()) || Player->GetName()==m_DefaultOwner)
									isAdmin = true;								
								if (Player->GetName()==User){
									SendChat( player->GetPID(), "You can't kick yourself!");
									return HideCommand;
								}
							} else { if ( m_Slots[SID-1].GetSlotStatus( ) == SLOTSTATUS_OCCUPIED && m_Slots[SID-1].GetComputer( ) == 1 ) {							
							...
							if (isAdmin)
							{
								...
								if ( GetNumHumanPlayers( ) < 5 || GetSlotsClosed( ) > 5 || m_Map->GetMapType( ).find("slotfix") != string::npos || m_GHost->m_FixedSlots ){
				...
				if( Command == "closeall" && !m_GameLoading && !m_GameLoaded )
				{
					if ( GetNumHumanPlayers( ) < 4 || GetSlotsClosed( ) > 6 || m_Map->GetMapType( ).find("slotfix") != string::npos || m_GHost->m_FixedSlots ){
						SendChat( player->GetPID(), m_GHost->m_Language->CommandDisabled( ));
						return HideCommand;
					}
					CloseAllSlots( );
					...
					while( !SS.eof( ) )
					{
						...
						else
						{
							...
							if (isAdmin)
							{
								...
								if ( GetNumHumanPlayers( ) < 4 || GetSlotsOpen( ) > 5 || m_Map->GetMapType( ).find("slotfix") != string::npos || m_GHost->m_FixedSlots ){
									SendChat( player->GetPID(), m_GHost->m_Language->CommandDisabled( ));
									return HideCommand;
								}
									OpenSlot( (unsigned char)( SID - 1 ), true );							
				...
				if( Command == "openall" && !m_GameLoading && !m_GameLoaded )
				{
					if ( GetNumHumanPlayers( ) < 4 || GetSlotsOpen( ) > 6 || m_Map->GetMapType( ).find("slotfix") != string::npos || m_GHost->m_FixedSlots ){
						SendChat( player->GetPID(), m_GHost->m_Language->CommandDisabled( ));
						return HideCommand;
					}
					OpenAllSlots( );
				...
				if( Command == "obsplayer" && !m_CountDownStarted && !m_GHost->m_ObsPlayerName.empty() && m_Map->GetMapType( ).find("slotfix") == string::npos && !m_GHost->m_FixedSlots )
				{
					if( m_ObsPlayerPID == 255 )
						CreateObsPlayer( false );
					else
				...
				{		
					if (DownloadInProgress( )) {
						SendChat(player->GetPID(),"Try "+string( 1, m_GHost->m_CommandTrigger )+"startn again when no map download");
						return HideCommand;
					}
					if(m_GHost->m_FakePlayersLobby && !m_FakePlayers.empty())					
						DeleteFakePlayer();
					...
					if ( m_GHost->m_StartGameWhenAtLeastXPlayers != 0){
						uint32_t AutoStartAtLeastPlayers = (m_GetMapNumPlayers < m_GHost->m_StartGameWhenAtLeastXPlayers) ? m_GetMapNumPlayers : m_GHost->m_StartGameWhenAtLeastXPlayers;
						if (GetNumHumanPlayers( ) < AutoStartAtLeastPlayers ){						
						SendChat(player->GetPID(), m_GHost->m_Language->AtLeastXPlayersToStartGame(UTIL_ToString(AutoStartAtLeastPlayers)));
						return HideCommand;
						}
					}
	...
	*********************/
	...
	//	
		...
		{						
			string s = m_GHost->m_BlacklistedNames+m_BannedNames;
			if(IsBlacklisted(s,User)){
				player->SetSocket( NULL );
				player->SetDeleteMe( true );
				player->SetLeftReason( "Kicked due to blacklisted names" );
				return HideCommand;
			}
			if ( m_Locked || m_NoTempOwner ) {
				SendChat( player->GetPID(), "TempOwner (!owner command) is currently disabled by admin");
				return HideCommand;
			}
	...
	//
		...
		if( Command == "country" || Command == "c" || (!m_GHost->m_CityPatch && (Command == "f" || Command == "from")) )
		{
			string Froms;
			string Froms2;
			string CNL;
...
//				CN = m_GHost->m_DB->FromCheck( UTIL_ByteArrayToUInt32( (*i)->GetExternalIP( ), true ) );
	...
	// !F
	//
		if(m_GHost->m_CityPatch)		
		if( Command == "from" || Command == "f" )
		{
			if (GetTime()>=m_LastFromInfoShow+30){
				m_LastFromInfoShow = GetTime( );
				string Froms;
				string Froms2;
				string CNL;
				string CN;
				...
				if (samecountry)
					...
					SendAllChat( Froms );
			} else SendChat( player, "Sup, 30s break applied. Give bot a little break :( Try again after 30s" );
		} */
	...
	if (GetTime()>=m_LastRuleInfoShow+30)
	{
		m_LastRuleInfoShow = GetTime( );
		// CONSOLE_Print("[GAME: " + m_GameName + "] loading bot_rule.txt");
		ifstream inn;
		inn.open( "bot_rule.txt" );

		if( !inn.fail( ) )
		{
			// don't print more than 16 lines

			uint32_t Count = 0;
			string Line;

			while( !inn.eof( ) && Count < 16 )
			{
				getline( inn, Line );
			...
			CONSOLE_Print("[GAME: " + m_GameName + "] bot_rule.txt load failed");

	} else SendChat( player, "Sup, 30s break applied. Don't rush the bot with command. What have you learned from "+string( 1, m_GHost->m_CommandTrigger )+"rule, now you spam? :P" );
	...
	if (GetTime()>=m_LastGuideInfoShow+30)
	{
		m_LastGuideInfoShow = GetTime( );
		// CONSOLE_Print("[GAME: " + m_GameName + "] loading bot_guide.txt");
		ifstream inn;
		inn.open( "bot_guide.txt" );
		...
		if( !inn.fail( ) )
		{
			...
			CONSOLE_Print("[GAME: " + m_GameName + "] bot_guide.txt load failed");

	} else SendChat( player, "Sup, 30s delay applied. Don't spam & rush the bot with command ._." );
	
	//
	//	!ABOUT
	//
	
	if( Command == "about" || Command == "abouts" || Command == m_GHost->m_UrOwnCommand )
	if (GetTime()>=m_LastAboutInfoShow+30)
	{
		m_LastAboutInfoShow = GetTime( );
		// CONSOLE_Print("[GAME: " + m_GameName + "] loading bot_aboutus.txt");
		ifstream inn;
		inn.open( "bot_aboutus.txt" );
		...
		if( !inn.fail( ) )
		{
			...
			CONSOLE_Print("[GAME: " + m_GameName + "] bot_aboutus.txt load failed");

	} else SendChat( player, "Sup, 30s delay applied. Don't spam & rush the bot with command :<" );
	...
	if (GetTime()>=m_LastFAQInfoShow+30)
	{
		m_LastFAQInfoShow = GetTime( );
		// CONSOLE_Print("[GAME: " + m_GameName + "] loading bot_faq.txt");
		ifstream inn;
		inn.open( "bot_faq.txt" );
		...
		if( !inn.fail( ) )
		{
			...
			CONSOLE_Print("[GAME: " + m_GameName + "] bot_faq.txt load failed");
	...
	if( ( Command == "stats" ) && GetTime( ) >= player->GetStatsSentTime( ) + 5 )
	{
		...
		if( player->GetSpoofed( ) && ( AdminCheck || RootAdminCheck || TempOwnerCheck ) )
	...
	if( ( Command == "statsdota" || Command == "ds" || Command == "dotastats" ) && GetTime( ) >= player->GetStatsDotASentTime( ) + 5 && !m_GHost->m_nostatsdota )
	{
		string StatsUser = User;
...
//			SendChat( player, m_GHost->m_Language->VersionNotAdmin( m_GHost->m_Version ) );
	...
	if( Command == "eat" )
	{
        ...
        if(numCookies > 0) {
            ...
            SendAllChat( "[" + User + "] has eaten a cookie. That was tasty! [" + User + "] now has " + UTIL_ToString(numCookies) + " cookies remaining.");
        } else {
            SendChat( player, "Error: you have no cookies in your cookie jar. Ask an admin to give you some with "+string( 1, m_GHost->m_CommandTrigger )+"cookie.");
        }
        HideCommand = true;
	}
	...
	bool votestartAuth = player->GetSpoofed( ) && ( AdminCheck || RootAdminCheck || TempOwnerCheck );
	...
	if( ( Command == "votestart" || Command == "vs" ) && !m_CountDownStarted && (votestartAuth || votestartAutohost || !m_GHost->m_VoteStartAutohostOnly))
	{
        if( !m_GHost->m_CurrentGame->GetLocked( ) )
		{
			if(m_StartedVoteStartTime == 0) { //need >minplayers or admin to START a votestart
				...
				if ( m_GHost->m_StartGameWhenAtLeastXPlayers != 0){
					uint32_t AutoStartAtLeastPlayers = (m_GetMapNumPlayers < m_GHost->m_StartGameWhenAtLeastXPlayers) ? m_GetMapNumPlayers : m_GHost->m_StartGameWhenAtLeastXPlayers;
					if ( GetNumHumanPlayers( ) < AutoStartAtLeastPlayers ){						
					SendChat( player->GetPID(), "You cannot use " +string( 1, m_GHost->m_CommandTrigger )+ "votestart until there're at least" + UTIL_ToString(AutoStartAtLeastPlayers) + " or more players!");
					return HideCommand;
					}      
				}
				for( vector<CGamePlayer *> :: iterator i = m_Players.begin( ); i != m_Players.end( ); ++i )
	...
	if( ( Command == "votekick" || Command == "vk" ) && m_GHost->m_VoteKickAllowed && !Payload.empty( ) )
	{
	...
	if( ( Command == "yes" || Command == "y" ) && !m_KickVotePlayer.empty( ) && player->GetName( ) != m_KickVotePlayer && !player->GetKickVote( ) )
	{
		player->SetKickVote( true );
	...
	// !WIM
	//
	if (!IsMuted(User) && !player->GetMuted())
	...
	//
	if (!IsMuted(User) && !player->GetMuted())
	if( ( Command == "ready" || Command == "rdy" || Command == "go" ) && !m_CountDownStarted && !m_GameLoaded && !m_GameLoading )
	{
		if (!player->GetStartVote( ))
		{
			if (!m_StartVoteStarted)
			{
    ...
    if( Command == "rmkcounter" && m_GameLoaded && !IsMuted(User) && !player->GetMuted() ) 
	{
      //count RMK's
	...
	if( (Command == "rmk" || Command == "remake") && m_GameLoaded && player->CanRMK() && !IsMuted(User) && !player->GetMuted() ) 
	{
		if(player->GetHasRMKed())  
		{
		...
		uint32_t Count = (CountRMK>4) ? (CountRMK - 2) : (CountRMK - 1);
		CountRMK = CountRMK - 1;		
		if(player->GetHasRMKed()) 
		{
			if (!team_p) {
		...
		if(VotesRMK >= Count && CountRMK!=0) // has at least a player
		{     
		  //game_RMK=true;  //disable autoban //autosave
			...
			{
				m_GHost->m_RemakeGame = m_GameName;
				CONSOLE_Print( "[GAME: " + m_GameName + "] ending by rmkvote (admin can end the game now with !endn)" );
...
//		player->SetRMKSentTime( GetTime( ) );
    ...
    if( Command == "ffcounter" && m_Stats && m_GameLoaded ) 
	{
      //count ff's
        ...
        SendChat(player,m_GHost->m_Language->FFCounter(string( 1, m_GHost->m_CommandTrigger )+"FF Sentinel")+": " + UTIL_ToString( VotesSentinel1 ) + "/" + UTIL_ToString( CountSentinel1) + ", "+m_GHost->m_Language->FFCounter(string( 1, m_GHost->m_CommandTrigger )+"FF Scourge")+": " + UTIL_ToString( VotesScourge1 ) + "/" + UTIL_ToString( CountScourge1 ));
	...
	if( (Command == "ff" || Command == "forfeit") && m_Stats && m_GameLoaded && player->CanFF() ) 
	{
		if(player->GetHasFFed())  
		{
		...
		for( vector<CGameSlot> :: iterator i = m_Slots.begin( ); i != m_Slots.end( ); i++ )
		{
			// observers cannot forfeit! 28-02-2013 adding i->GetTeam()!=12 
		...
		if(player->GetHasFFed()) 
		{
			if (!team_p) {
			//sentinel
				SendAllChat( "Player [" + User + "] gave FF command. "+m_GHost->m_Language->FFCounter("Sentinel")+": " + UTIL_ToString( VotesSentinel ) + "/" + UTIL_ToString( CountSentinel));
			} else {
			//scourge
				SendAllChat( "Player [" + User + "] gave FF command. "+m_GHost->m_Language->FFCounter("Scourge")+": " + UTIL_ToString( VotesScourge ) + "/" + UTIL_ToString( CountScourge ));
			}
		} else
		{
            if (!team_p) {
				SendAllChat( m_GHost->m_Language->FFOath(User)+" "+m_GHost->m_Language->FFCounter("Sentinel")+": " + UTIL_ToString( VotesSentinel ) + "/" + UTIL_ToString( CountSentinel ));
			} else {
				SendAllChat( m_GHost->m_Language->FFOath(User)+" "+m_GHost->m_Language->FFCounter("Scourge")+": " + UTIL_ToString( VotesScourge ) + "/" + UTIL_ToString( CountScourge ));
			};
		} 
		if((VotesSentinel == CountSentinel) && (CountSentinel!=0) ) // has at least a player
		{
		  ...
		  //game_ff=true;  //disable autoban //autosave
			SendAllChat(m_GHost->m_Language->FFResignAndWin( "Sentinel", "Scourge" ));
			if (!m_GameEndCountDownStarted)
						{
		...
		else if((VotesScourge == CountScourge) && (CountScourge!=0)) // has at least a player
		{
			m_Stats->SetWinner(1);
			//game_ff=true;     //disable autoban  //autosave
			SendAllChat(m_GHost->m_Language->FFResignAndWin( "Scourge", "Sentinel" ));
			if (!m_GameEndCountDownStarted)
						{
							CONSOLE_Print( "[GAME: " + m_GameName + "] ending (admin can end the game now with !endn)" );
							SendAllChat("Game over in 10 secs automatically. YOU CAN LEAVE NOW!");
...
//		player->SetFFSentTime( GetTime( ) );
	...
	if( (Command == "sd" || Command == "sdi" || Command == "sdpub" || Command == "sdpriv") && GetTime( ) >= player->GetStatsDotASentTime( ) + 5 )
	{
		...
		bool nonadmin = !(( player->GetSpoofed( ) &&  AdminCheck) || RootAdminCheck || TempOwnerCheck ) ;
